...
Like `banner` and `description`, `doubleImageDescription` is a content block: it is not a top-level `layout.templates` entry itself, it lives inside a `description`/`multiDescriptions` template's `content` map (the key, e.g. `description-1`, is stripped of its trailing `-N` to resolve the folder, and `type` selects the component file).
